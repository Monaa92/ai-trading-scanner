...
Status: **IN PROGRESS — FOUNDATION/SCHEDULER/ORCHESTRATION ACCEPTED; MILESTONE 6.1 ORDER/TERMINAL RESOLVER CANDIDATE IMPLEMENTED AND AWAITING INDEPENDENT REVIEW.**
...
| Simulation execution | MARKET-only full-fill contract plus unreviewed 6.1 implementation: authoritative idempotent order creation, versioned projection, strict later same-session candidates, explicit full-fill volume-participation profile, engine-stamped cancellation and deterministic fill-ready/expired/no-data/cancelled/rejected evidence | Independent 6.1 review; then fill creation, manifest-level liquidity binding, cost/FX, reservation lifecycle, accounting and fixed stop/target remain |
