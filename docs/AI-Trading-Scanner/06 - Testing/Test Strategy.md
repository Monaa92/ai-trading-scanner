...
Status: Phase 1–5 suites are complete on `main`. Phase 6 is **IN PROGRESS** with focused contract tests; complete causal replay, portfolio transitions, simulation P&L and frontend tests remain future work. See [PROJECT_RULES](../../PROJECT_RULES.md), [[06 - Testing/Phase 5 Independent Review]], [[06 - Testing/Phase 6 Independent Review]] and [[06 - Testing/Phase 6 Completion Criteria]].
...
The Phase 6 foundation has 79 focused contract cases. The original 58 cover content-stable run/execution/cost/event/order/fill/portfolio/result identities; SIMULATION/replay-only dimensions; independent FULL_AUTO semantics; timezone and causal ordering failures; future market availability; nonnegative routing latency with strictly later execution opens; next-interval fill lineage; float and invalid financial rejection; unsupported partial/random assumptions; exact itemized costs; order/fill attribution; cash/position/equity conservation; position/trade lifecycle contracts; canonical same-time phase order and phase/payload compatibility; duplicate/mixed/out-of-order trace rejection; deterministic NDJSON bytes; and forbidden broker/provider/network/AI imports. Twenty-one remediation cases cover scaled, signed-zero and scientific Decimal equivalence; payload-aware causal scheduling; missing, foreign and phase-substituted artifacts; COMPLETE reconciliation and derived result linkage; deterministic semantic tie ordering; a valid Phase 4→6 execution chain; and fail-closed cost model, execution model, approved quantity/side, fill price and next-bar mismatches. These tests do not claim a scheduler, adapter, durable posting, durable store or completed simulation exists.
