...
The scheduler increment adds 29 focused cases for increasing/equal timestamps, phase and semantic tie precedence, simultaneous instruments, shuffled/repeated/JSON-reconstructed schedules, sequential and concurrent exact-once consumption, deterministic exhaustion, duplicate/conflicting/cross-run and malformed event rejection, checkpoint reconstruction and mismatch rejection, consumed-prefix integrity, mutation resistance, market-availability and accounting-chain position, artifact-registry independence, typed artifact-linkage revalidation and content-sensitive identities. These tests prove scheduler envelope sequencing and in-memory state only. They do not claim causal payload orchestration, an adapter, transactional/durable posting, crash recovery, external marking, a complete portfolio lifecycle or completed simulation exists.
