...
Status: Phase 1–5 suites are complete on `main`. Phase 6 is **IN PROGRESS** with focused contract, reconciliation, deterministic scheduler and causal-orchestration tests; transactional execution/accounting posting, durable replay, complete lifecycle, the four-agent historical fixture, full experiment P&L and frontend tests remain future work. See [PROJECT_RULES](../../PROJECT_RULES.md), [[06 - Testing/Phase 5 Independent Review]], [[06 - Testing/Phase 6 Independent Review]] and [[06 - Testing/Phase 6 Completion Criteria]].
...
Phase 5 now has 177 focused cases. The original 68 cover typed ownership/content identities; research-policy validation; float/non-finite/LIVE rejection; exact sizing/downward rounding; final-quantity immutability; risk, position, cash, agent, parent and concentration caps; capital scaling; proposal time/configuration/currency/ownership; structured rejections; reservation conservation/lifecycle; duplicate/idempotent behavior; approval binding; scoped locks; rollback; and core races. The first 27 remediation cases add eligible-equity and daily-headroom boundaries, current loss-state revision, outstanding downside lifecycle, stale-preflight revalidation, proposal uniqueness, exact expiry replay/consume behavior, lock-before-replay, sizing arithmetic, Decimal enforcement, safety-decision identity and cross-parent lock concurrency. The second 21 adversarial cases cover sequential/concurrent same-agent allocation rejection, downside/exposure/count fragmentation prevention, post-lifecycle allocation identity, coherent proposal/configuration/allowed-risk forgery rejection, legitimate JSON reconstruction, cross-scope loss misuse, session/freshness/causal timestamp failures, revision/time regression and final freshness revalidation. The third 15 cases cover exact and overflowing parent allocation ownership, atomic/concurrent registration, ownership persistence across reservation terminal states, registration/reservation locking, deterministic quantity provenance and rejected-decision source/safety/session relationships. The fourth eight cases force parent/allocation publication races before and after primary-snapshot insertion with many readers, verify unpublished and failed scopes leave no observable residue, and reserve immediately after registration. The fifth 38 cases inject failures before and after all six reservation and all four lifecycle publication stages for RELEASED/EXPIRED/CONSUMED, verify exact reverse rollback and successful retries, and use deterministic barriers to exclude intermediate reader/competitor visibility. Tests scan the `risk` package AST for forbidden broker/provider/network/AI imports. The complete local fifth-remediation workspace passes 480 tests, including 303 Phase 1–4 regressions.

The Phase 6 foundation has 116 focused cases. The original 58 cover content-stable run/execution/cost/event/order/fill/portfolio/result identities; SIMULATION/replay-only dimensions; independent FULL_AUTO semantics; timezone and causal ordering failures; future market availability; nonnegative routing latency with strictly later execution opens; next-interval fill lineage; float and invalid financial rejection; unsupported partial/random assumptions; exact itemized costs; order/fill attribution; cash/position/equity conservation; position/trade lifecycle contracts; canonical same-time phase order and phase/payload compatibility; duplicate/mixed/out-of-order trace rejection; deterministic NDJSON bytes; and forbidden broker/provider/network/AI imports. The first 21 remediation cases cover Decimal equivalence, typed payload/derived-result linkage, semantic ordering and the Phase 4→6 chain. The second 28 cases specifically reject unchanged/missing/cost-omitting or otherwise forged final portfolios, duplicate/missing/foreign fill application, wrong realized results, coherently reidentified reservation-economic tampering, dangling or contradictory terminal resolutions and next-session execution; they accept exact reservation economics, reconciled open/closed accounting, and regular/early-close/DST same-session selection. The third nine cases cover equal-time fill/application/checkpoint dependencies, multiple simultaneous fills and instruments, wrong-transition checkpoints, duplicate applications, phase precedence, canonical shuffled registries and repeated byte/identity determinism.

The scheduler remediation has 47 focused cases. They cover authoritative ordering, equal-time phase/dependency precedence, simultaneous instruments, shuffled valid artifacts, schedule identity, sequential/concurrent exact-once consumption, shared process-local cursor state across concurrent handles, exhausted-handle anti-replay and deterministic exhaustion. Trust-boundary cases prove unresolved raw events and independently reconstructed schedules cannot become executable; duplicate/missing/foreign typed payloads, manifest mismatch and event/payload causal-time mismatch fail through authoritative `ReplayArtifactBundle` validation. Checkpoint cases cover exact issued state, JSON reconstruction within the issuing process, arbitrary middle/final/zero positions, fully recomputed content identities, changed/reordered prefixes, wrong next event/schedule/run/length, foreign schedules, stale checkpoints and equivalent shuffled artifacts. Content identity alone does not grant resume trust. These tests prove scheduler envelope sequencing and in-memory anti-skip/replay only. They do not claim cross-process anti-rollback, durable recovery, causal payload orchestration, transactional posting, external marking, a complete portfolio lifecycle or completed simulation exists.

The causal-orchestration increment now has 77 focused cases. The original 25 cover consumed-prefix visibility, same-time isolation, fresh-process identities, NO_TRADE, all four strategies, Phase 5 proposal outcomes, ownership, configuration and offline operation. Thirty-six first-remediation cases cover exclusive cursor leases, concurrent handles, exception-atomic rollback at every reachable transition boundary, authoritative artifact/schedule/configuration provenance and valid reconstruction. Sixteen final provenance cases alter and reidentify EMA/RSI/ATR/VWAP evidence, the strategy decision, post-reservation capital, reservation amount/identity, capital revisions, foreign Phase 5 evidence, outcome status and rejection reasons; every fabrication fails authoritative validation. Legitimate NO_TRADE, risk rejection and reservation reconstruction remain deterministic, and normal advancement evaluates the strategy once. The full workspace passes 720 tests: 480 Phase 1–5 regressions and 240 Phase 6 cases.
