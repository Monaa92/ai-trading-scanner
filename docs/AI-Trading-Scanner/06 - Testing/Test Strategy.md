...
The causal-orchestration increment adds 25 focused cases. They cover consumed-prefix visibility, a future bar already present in the artifact remaining hidden, current-bar release, identical-timestamp isolation, shuffled registry canonicalization, fresh-process deterministic identities, typed result reconstruction, first-class NO_TRADE without allocation mutation, real Phase 4 proposal generation reaching Phase 5, accepted sizing/reservation evidence, deterministic submission/risk rejection, wrong run/agent/allocation and unresolved dataset rejection, forged unseen lineage, equivalent normalized evidence for all four strategies, no privileged Multi-Factor raw input, immutable scheduler history, exactly-once exhaustion, shared visibility across duplicate handles, configuration-sensitive identities, complete causal-parent linkage and network-free operation.
