...
Phase 5 adds 68 focused cases for typed ownership and content identities; research-policy validation; float/non-finite/LIVE rejection; exact sizing and downward rounding; final-quantity immutability; risk, position, cash, agent, parent and concentration caps; capital-scaling invariants; proposal time/configuration/currency/ownership checks; first-class structured rejections; reservation creation/conservation; release/consume/expiry; duplicate/idempotent behavior and exact replay evidence; exact approval binding; local/parent locks; rollback; and concurrent same-agent, shared-parent, duplicate, release and lock races. Tests scan the `risk` package AST for forbidden broker/provider/network/AI imports. The complete local candidate passes 371 tests.
