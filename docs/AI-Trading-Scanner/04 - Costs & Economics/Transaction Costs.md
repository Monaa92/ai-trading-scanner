...
The V1 foundation stores the next eligible same-session bar open as gross fill price and represents spread/slippage as separate monetary drag. The pure COMPLETE reconciler derives portfolio cash and net P&L with each fill-cost component debited once; callers must not also shift the fill price for the same configured spread/slippage. A transactional posting service is still unimplemented. This convention is versioned by [[00 - Project/Decisions/ADR-033 - Deterministic Phase 6 replay foundation]]. Cost rounding, entry-versus-exit asymmetry, nonlinear tiers, regulatory sell-only fees, FX and reconciliation from Phase 4 expected round-trip return drag to realized amounts remain unimplemented.
