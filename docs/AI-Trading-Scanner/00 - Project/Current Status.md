...
**Current phase:** Phase 6 — causal backtesting and simulation, **IN PROGRESS: CONTRACT FOUNDATION ONLY**. Immutable run manifests, market-event references, replay envelopes, simulation execution/cost configuration, market orders/fills, portfolio/position/result records and canonical NDJSON serialization are implemented. The scheduler, strategy/risk orchestration, order lifecycle, fill-to-ledger transitions and durable artifact writer are not implemented.
...
| Phase 6 causal backtesting/simulation | IN PROGRESS — FOUNDATION | Content-identified causal contracts, V1 phase ordering, strict later next-bar-open fill policy, versioned exact cost components, single-currency Decimal portfolio conservation and canonical event serialization. Focused tests cover contracts only. See [[06 - Testing/Phase 6 Completion Criteria]]. |
...
Next recommended action: implement the next bounded Phase 6 increment: causal scheduler and ordered Phase 2 market-availability replay into Phase 3 indicators, retaining prefix invariance and the versioned event trace. Do not start Phase 7.
