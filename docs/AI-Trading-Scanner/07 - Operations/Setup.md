...
The initial repository had no production code, package manifest, tests or migrations. Phase 1 added the offline executable safety foundation; Phase 2 added historical market-data records, calendar/session semantics, provenance, content identity, quality validation, fixtures and causal reads. Phase 3 adds deterministic indicators. The Phase 4 review candidate adds pure strategy evaluation and proposal contracts only. No provider adapter, data download, database, scanner, central risk, portfolio, broker or order-execution component exists. Existing Obsidian app/appearance/core-plugins/workspace JSON remains preserved. No duplicate vault exists and no active link or configuration points to the former vault path.
