...
The initial repository had no production code, package manifest, tests or migrations. Phase 1 added the offline executable safety foundation; Phase 2 added historical market-data records and causal reads; Phase 3 adds deterministic indicators; Phase 4 adds pure strategy/proposal contracts; and the Phase 5 candidate adds pure risk sizing plus local in-memory reservation contracts. No provider adapter, data download, database, scanner, full portfolio/P&L ledger, broker or order-execution component exists. Existing Obsidian app/appearance/core-plugins/workspace JSON remains preserved. No duplicate vault exists and no active link or configuration points to the former vault path.
