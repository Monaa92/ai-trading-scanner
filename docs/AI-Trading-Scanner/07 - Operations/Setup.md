...
The initial repository had no production code, package manifest, tests or migrations. Phase 1 added the offline executable safety foundation; Phase 2 added historical market-data records and causal reads; Phase 3 adds deterministic indicators; Phase 4 adds pure strategy/proposal contracts; Phase 5 adds accepted risk sizing and local in-memory reservation contracts; and the Phase 6 foundation adds immutable simulation/accounting contracts and deterministic event serialization. No scheduler, provider adapter, data download, database, scanner, durable portfolio/P&L ledger, broker or external order-execution component exists. Existing Obsidian app/appearance/core-plugins/workspace JSON remains preserved. No duplicate vault exists and no active link or configuration points to the former vault path.
