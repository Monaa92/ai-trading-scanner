...
[[01 - Architecture/Execution/Replay and Simulation Architecture]] specializes these invariants for offline SIMULATION. A simulated order is immutable internal evidence and never enters a broker outbox. The initial executable profile uses market orders, full fill or one typed terminal no-fill, next eligible same-session bar open, fixed stop/target and conservative stop-first OHLC ambiguity. Order, reservation, fill, cash/cost/FX, position and cursor effects eventually share one atomic journal transition. None of that runtime lifecycle is implemented by this documentation change.
