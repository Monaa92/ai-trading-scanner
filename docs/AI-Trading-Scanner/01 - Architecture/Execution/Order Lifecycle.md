...
[[01 - Architecture/Execution/Replay and Simulation Architecture]] specializes these invariants for offline SIMULATION. A simulated order is immutable internal evidence and never enters a broker outbox. The unreviewed milestone 6.1 remediation implements only authoritative cursor-bound idempotent order creation, one exception-atomic in-memory projection root, engine-stamped cancellation and resolver-authoritative fill-ready/terminal evidence from the released causal prefix. It uses a V2 run manifest containing explicit versioned full-fill volume participation and creates no `SimulatedFill`. Reservation, fill, cash/cost/FX, position and cursor effects still require one future atomic journal transition; fixed stop/target and accounting behavior remain unimplemented.
