...
[[02 - Agents & Strategies/Autonomous Survival Agent]] defines a separate future reasoning use case. A provider-independent model may propose policy, organization, allocation or trade actions, but every output remains an immutable untrusted `ModelDecisionEnvelope`. The deterministic engine validates and applies only registered actions. Historical replay consumes the captured output rather than recalling the provider. This architecture does not implement a provider or expand the current FILTER contract.
