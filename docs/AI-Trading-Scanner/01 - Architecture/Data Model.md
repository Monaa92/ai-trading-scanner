...
| IndicatorSeries/Point (I) | Dataset/slice/configuration hashes, instrument/timeframe/session, bar/availability times, exact value or null, readiness reason and quality warnings | Phase 3 in-memory implementation; one point per causal input bar, no persistence yet |
| StrategyConfiguration (I) | Strategy/profile/version, five-minute timeframe, indicator periods, freshness/economic thresholds and explicit profile parameters | Phase 4 in-memory, content-identified `BASELINE_RESEARCH_V1`; research status separate from identity |
| StrategyDecision (I) | Decision ID, outcome, agent/strategy/config/instrument/as-of, dataset/slice/indicator IDs, structured evidence, quality findings and NO_TRADE reasons or proposal | Phase 4 in-memory; exactly one outcome, no null non-trade |
| TradeProposal/ManagementMandate (I) | Proposal/mandate IDs, long intent, completed-bar reference, future-risk sizing intent, stop, objective, management triggers, gross/cost/net economics, expiry and authority context | Phase 4 in-memory approval-sensitive content identity; no approval, reservation, order, fill or position |
| FeatureSnapshot (I) | Instrument/as_of, values/units/readiness/reasons, indicator+feature versions, parameters, input revision IDs/hash | Planned scanner-facing aggregation; fully traceable to bars/actions; new revisions create new snapshots |
