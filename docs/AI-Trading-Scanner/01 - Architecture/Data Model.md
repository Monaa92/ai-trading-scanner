...
The persistence schema remains a specification: no DDL, database or migrations exist. Phase 2 implements in-memory Python `Instrument`, `HistoricalBar`, `DataProvenance`, `CanonicalDataset`, quality-result and causal-slice records plus typed `InstrumentId`/`DatasetId`. The Phase 3 review candidate adds immutable indicator configurations, `IndicatorConfigurationId`, points and series; these are not persisted yet. PostgreSQL is intended for later transactional records and metadata; large immutable dataset artifacts may later live in private object/file storage referenced by checksum. Hosting, retention sizes and physical partitioning await measured data volume. See [[01 - Architecture/Market Data]], [[02 - Agents & Strategies/Shared Agent Rules/Indicators]], [[07 - Operations/Configuration]] and [[07 - Operations/Logging & Observability]].
...
| IndicatorSeries/Point (I) | Dataset/slice/configuration hashes, instrument/timeframe/session, bar/availability times, exact value or null, readiness reason and quality warnings | Phase 3 in-memory review candidate; one point per causal input bar, no persistence yet |
