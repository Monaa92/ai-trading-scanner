...
The immutable sizing artifact embeds the complete self-validating Phase 4 `TradeProposal` and Phase 5 `RiskConfiguration` that produced it. It requires their content identities to match its attribution and validates entry, stop, currency, transaction-cost return, quantity increment, exact final-quantity intent, allowed-risk derivation and all reconstructible multiplication/addition equalities. An externally reconstructed artifact that changes financial values coherently while retaining the old proposal/configuration attribution therefore fails validation. Normal JSON serialization/deserialization retains and revalidates this evidence.
