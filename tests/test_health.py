...
def test_health_cli_succeeds_offline(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert report["deterministic_indicators_validated"] is True
    assert report["strategy_contracts_validated"] is True
    assert report["risk_contracts_validated"] is True
