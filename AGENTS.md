...
Current phase: Phases 1–5 are complete on `main`. Phase 6 — causal backtesting and simulation — is **IN PROGRESS** on `feat/phase-6-causal-simulation`. The immutable simulation foundation, deterministic scheduler and bounded causal orchestration through `b76891f7a1d787c2c07d26c0f4dd6a42df55aada` are accepted. The target replay/execution, causal cost/FX, durable recovery and future 4+1/Autonomous Survival Agent architecture are documented but unimplemented. Complete simulated fills, fill/accounting orchestration, durable artifacts, the full position lifecycle, four-agent runner and Agent 5 do not exist. `BASELINE_RESEARCH_V1` remains an unvalidated research policy. No scanner, broker/provider/AI integration, deployment, PAPER/LIVE execution or external order authority is implemented.
