...
Current phase: Phases 1–5 are complete on `main`. Phase 6 — causal backtesting and simulation — is **IN PROGRESS** on `feat/phase-6-causal-simulation`. Its foundation adds immutable causal run/event/order/fill, versioned execution/cost, Decimal portfolio and result/serialization contracts. It does not implement the scheduler, fill/accounting orchestration, durable artifact writer, full position lifecycle or four-agent runner. `BASELINE_RESEARCH_V1` remains an unvalidated research policy. No scanner, broker/provider/AI integration, deployment, PAPER/LIVE execution or external order authority is implemented.
