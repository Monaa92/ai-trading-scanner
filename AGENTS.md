...
Current phase: Phases 1–5 are complete on `main`. Phase 6 — causal backtesting and simulation — is **IN PROGRESS** on `feat/phase-6-causal-simulation`. The immutable simulation foundation, deterministic scheduler and bounded causal orchestration through `b76891f7a1d787c2c07d26c0f4dd6a42df55aada` are accepted. Independent review rejected milestone 6.1 candidate `bac6b0db268c6eb39b8f1fd9a80deb2d5fa2e525`; its bounded remediation (`4beecc5728ff40cb4b12d69cc71c74f8f4ac09f5`) is committed and pushed on this feature branch. It binds creation to the current causal cursor, publishes resolver registries atomically, validates terminal evidence against resolver authority, never future-dates end-of-data outcomes and freezes liquidity in V2 run identity. It has no fill/accounting mutation authority. Claude's independent technical review (2026-09-18) found the six previously reproduced defects resolved and full verification passing (772 tests, ruff, strict mypy); independent competent human reviewer `Dekkerszz` then approved it via GitHub PR #5 (2026-09-18) within the stated Phase 6.1 scope, so Milestone 6.1 is ACCEPTED. Two low-severity observations remain open, accepted follow-up items. This approval does not authorize merging PR #5 or the feature branch into `main`. On `feat/phase-6.2-cost-fx-foundation`, the first bounded Milestone 6.2 step — pure, evidence-only `CostProfileRegistration`/`CostRoundingPolicy` and `FxObservationReference`/`FxConversionPolicyConfiguration`/`FxQuoteConvention` contracts, per [[00 - Project/Decisions/ADR-036 - Phase 6.2 cost and FX foundation]] — is implemented and under independent review; it is not yet accepted. Later Phase 6.2 work (the causal FX resolver, balanced-leg calculation, `SimulationRunManifest` V3 integration, and any fill/accounting posting) still requires separate explicit owner authorization before it may begin. Complete simulated fills, reservation consumption/release, cost/FX posting, accounting orchestration, durable artifacts, the full position lifecycle, four-agent runner and Agent 5 do not exist. `BASELINE_RESEARCH_V1` remains an unvalidated research policy. No scanner, broker/provider/AI integration, deployment, PAPER/LIVE execution or external order authority is implemented.
