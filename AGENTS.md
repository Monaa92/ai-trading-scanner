...
Current phase: Phases 1–4 are complete on `main`. PR #4's original Phase 5 candidate received CHANGES REQUIRED; the remediation candidate implements deterministic Decimal risk sizing against content-identified current loss/headroom evidence, proposal-global duplicate protection, atomic in-memory reservations, expiry-safe replay/lifecycle behavior, partitioned scoped locks and immutable proposal/approval binding. Independent competent human re-review is required before merge. `BASELINE_RESEARCH_V1` remains an unvalidated research policy. No scanner, full portfolio/P&L ledger, durable persistence, broker/AI integration, deployment, order submission or trading automation is implemented.
