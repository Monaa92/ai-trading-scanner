...
Current phase: Phases 1–5 are complete on `main`. Phase 6 — causal backtesting and simulation — is **IN PROGRESS** on `feat/phase-6-causal-simulation`. The immutable simulation foundation and deterministic scheduler are accepted. The bounded causal-orchestration increment is in progress; complete replay/execution, simulated fills, fill/accounting orchestration, durable artifacts, the full position lifecycle and the four-agent runner are not implemented. `BASELINE_RESEARCH_V1` remains an unvalidated research policy. No scanner, broker/provider/AI integration, deployment, PAPER/LIVE execution or external order authority is implemented.
