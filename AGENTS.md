...
Current phase: Phases 1–4 are complete on `main`. A Phase 5 review candidate implements deterministic Decimal risk sizing, typed parent/agent ownership, atomic in-memory reservations, idempotent release/consume/expiry, scoped safety locks and immutable proposal/approval binding. Independent competent human review is required before merge. `BASELINE_RESEARCH_V1` remains an unvalidated research policy. No scanner, full portfolio/P&L ledger, durable persistence, broker/AI integration, deployment, order submission or trading automation is implemented.
